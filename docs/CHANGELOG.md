...
- [改进] 搜索引擎第 5 优先级由 MiniMax 替换为百炼 (DashScope) 联网搜索，环境变量 `MINIMAX_API_KEYS` 替换为 `BAILIAN_API_KEYS`。
...
- [修复] 业绩预期数据缺失问题：接入 Tushare 财报 API（业绩预告 forecast、财务指标 fina_indicator、利润表 income、业绩快报 express），替换不可靠的 AkShare/SerpAPI 搜索，并调整 fundamental 超时配置（stage 1.5s→15s, fetch 0.8s→10s）以适应多 API 调用。
...
- [新功能] 历史分析趋势展示：AnalysisHistory 新增 change_pct/volume_ratio/turnover_rate/index_csi2000_pct/index_chinext_pct 五个行情快照列；新增 /api/v1/history/trend 按天取最后一条的 API；前端报告详情底部展示历史趋势表格；邮件/通知报告追加历史趋势模块；中证2000指数数据源补充；新增回补脚本 scripts/backfill_history_metrics.py
