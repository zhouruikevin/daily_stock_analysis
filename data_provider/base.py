...
class DataFetcherManager:
    ...
    def get_main_indices(self, region: str = "cn") -> List[Dict[str, Any]]:
        ...
        for fetcher in self._fetchers:
            # 仅在 CN 区域跳过非实时源（CN 有 TickFlow/efinance/akshare 等盘中实时数据）；
            # HK/US 等区域 YfinanceFetcher 才是主源，不应排除
            if region == "cn" and fetcher.name in self._NON_REALTIME_INDEX_FETCHERS:
                continue
            try:
